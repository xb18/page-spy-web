...
```java android
// Android 使用 OkHttp 的 `WebSocketListener`
// 宿主 App 需要引入 OkHttp，或改造成项目已有的 WebSocket 客户端。
...
```ts harmony
// Harmony 使用 NetworkKit 的 WebSocket API；
// 如果宿主 Harmony SDK 使用不同的 WebSocket 导入路径，只需要替换 import，保持 NativeModule 契约不变。
